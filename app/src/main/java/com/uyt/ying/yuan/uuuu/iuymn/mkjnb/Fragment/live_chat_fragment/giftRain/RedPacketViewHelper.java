...
public class RedPacketViewHelper {
    ...
    public interface GiftRainListener {
        ...
        void openBoom();//点击空包回调,(现在业务需求每次点击红包都是空包所以用于判断用户在红包雨期间是否有点击红包)
    ...
    public RedPacketViewHelper(SoftReference<Activity> activity) {
     ...
     * 获取礼物
     */
    public void openGift(int pos, BoxPrizeBean boxPrizeBean) {
...
//        mGiftRainView.setOnTouchListener(null);//出礼物后不处理点击。
     ...
     * 自定义获取礼物
     */
    public void cusOpenGift(BoxPrizeBean boxPrizeBean) {
        if (mGiftRainView == null || mRedPacketRender == null) {
            return;
        }
        if (mActivity == null || mActivity.isFinishing()) {
...
//        mGiftRainView.setOnTouchListener(null);//出礼物后不处理点击。
        //通知渲染器绘制礼物
        mRedPacketRender.cusOpenGift( boxPrizeBean);
        mGiftRainListener.openGift(boxPrizeBean);
    }
        ...
        mGiftRainView.setOnTouchListener((v, event) -> {
            if (event.getAction() == MotionEvent.ACTION_DOWN) {
                ...
                if (pos >= 0) {
                    ...
                    //需求变动, 红包雨中都是空包,红包雨结束之后再弹出抢包金额
/*                    if (random.nextInt(10) > 7) {
                        BoxInfo boxInfo = boxInfoList.get(pos);
                        BoxPrizeBean boxPrizeBean = new BoxPrizeBean();
                        boxPrizeBean.amount = 5;
                        boxPrizeBean.prizeName = Utils.getString(R.string.喵🐱);
                        openGift(pos, boxPrizeBean);
                    } else {*/
                        openBoom(pos);
                        if(null!=mGiftRainListener){
                            mGiftRainListener.openBoom();
                        }
//                    }
                    return true;
                }
