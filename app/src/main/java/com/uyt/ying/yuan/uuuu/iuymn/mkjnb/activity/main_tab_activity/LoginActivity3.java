...
/*    @BindView(R.id.account_loading_iv)
    ...
    private void initSharePreferencesData() {
        ...
        frontUrl = SharePreferencesUtil.getString(LoginActivity3.this, "frontUrl", "");
        domain = SharePreferencesUtil.getString(LoginActivity3.this, CommonStr.DOMAIN, "");
        mobileNumSwitch = SharePreferencesUtil.getString(LoginActivity3.this, "mobileNumSwitch", "");
        appSlidingCheck = SharePreferencesUtil.getString(LoginActivity3.this, "appSlidingCheck", "0");
            ...
            HttpApiUtils.CPnormalRequest(this, null, RequestUtil.CHECK_REGISTER, data, new HttpApiUtils.OnRequestLintener() {
                ...
                public void onSuccess(String result, Headers headers) {
                    ...
                    if(StringMyUtil.isNotEmpty(inviteCode)){
                       LoginActivity3.this.inviteCode = inviteCode;
                    }
        ...
        switch (view.getId()){
            ...
            case R.id.get_phone_code_tv:
                ...
                if(checkPhone(false)&&get_phone_code_tv.getText().toString().equals(SEND_PHONE_CODE)){
                    initSlideWebViewPop(true,false);
                }
                break;
...
//                    initSlideWebViewPop(false);
            ...
            case R.id.account_forget_passwoed_tv2:
                if(mobileNumSwitch.equals("0")){
                    startActivity(new Intent(LoginActivity3.this, PhoneFindPasswordActivity.class));
                }else {
                    startActivity(new Intent(LoginActivity3.this, AccountFindPasswordActivity.class));
                }
                break;
            case R.id.kefu_tv:
            case R.id.account_kefu_tv:
            case R.id.phone_kefu_tv:
                Intent intentKeFu = new Intent(LoginActivity3.this, OnLineKeFuActivity.class);
...
/*                ConstraintLayout v = phone_login_constraintLayout;
            ...
            case R.id.phone_register_btn:
                startActivity(new Intent(LoginActivity3.this, RegisterActivity.class));
                break;
    ...
    private void requestVisitorLogin() {
        ...
        HttpApiUtils.CpRequest(LoginActivity3.this, null, RequestUtil.VISITOR_LOGIN, data, new HttpApiUtils.OnRequestLintener() {
            @Override
            public void onSuccess(String result, Headers headers) {
                RequestLoginUtils.requestSuccess(LoginActivity3.this,result,account,password,"");
            }
    ...
    private void checkPhoneIsExit(boolean sendPhoneCode,String value) {
        ...
        HttpApiUtils.CpRequest(LoginActivity3.this, null,RequestUtil.FORGET_PASSWORD, data, new HttpApiUtils.OnRequestLintener() {
            @Override
            public void onSuccess(String result, Headers headers) {
    ...
    private void initSlideWebViewPop(boolean sendPhoneCode,boolean isVisitorLogin) {
        ...
        if(appSlidingCheck.equals("0")||isVisitorLogin){
            ...
            SlideWebViewPop slideWebViewPop = new SlideWebViewPop(LoginActivity3.this, true, poptype);
            slideWebViewPop.setSlideInterface(new SlideWebViewPop.SlideInterface() {
                ...
                public void onSlideListener(String tencent_randstr, String tencent_ticket) {
                    LoginActivity3.this.tencent_randstr = tencent_randstr;
                    LoginActivity3.this.tencent_ticket = tencent_ticket;
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            ProgressDialogUtil.darkenBackground(LoginActivity3.this,1f);
            ...
            ProgressDialogUtil.darkenBackground(LoginActivity3.this,0.5f);
        }else {
            if(sendPhoneCode){
                String toString = get_phone_code_tv.getText().toString();
                if(toString.equals(SEND_PHONE_CODE)){
    ...
    private void getIntentData() {
        ...
        if( !StringMyUtil.isEmptyString(nickName)){
            ...
            account_login_account_etv.setText(SharePreferencesUtil.getString(LoginActivity3.this, "account", ""));
        }
    }
    ...
    Runnable runnable =new Runnable() {
        ...
        public void run() {
            if(singleLogin){
                View view = LayoutInflater.from(LoginActivity3.this).inflate(R.layout.single_login_pop,null);
                ...
                singleLoginPop.setOnDismissListener(new PopupWindow.OnDismissListener() {
                    @Override
                    public void onDismiss() {
                        if(!LoginActivity3.this.isFinishing()&&!LoginActivity3.this.isDestroyed()){
                            ProgressDialogUtil.darkenBackground(LoginActivity3.this,1f);
                        }
                    }
                });
    ...
    private void loginSuccess(String message) {
        if (message.equals(Utils.getString(R.string.登入成功))) {
            SharePreferencesUtil.putString(LoginActivity3.this, "account", account);
            SharePreferencesUtil.putString(LoginActivity3.this, "password", password);
            SharePreferencesUtil.putString(LoginActivity3.this, "RemembePassword", password);
            SharePreferencesUtil.putBoolean(LoginActivity3.this, "isLogin", true);
//            if(cancelLogin){
...
//            }
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            Long user_id = SharePreferencesUtil.getLong(LoginActivity3.this, "user_id", 0l);
            switch (msg.what) {
                //请求融云信息
                case 1:
                    HashMap<String, Object> data = new HashMap<>();
                    ...
                    Utils.docking(data, RequestUtil.REQUEST_Chat1, 0, new DockingUtil.ILoaderListener() {
                        ...
                        public void success(String content)  {
                            ...
                            try {
                                ...
                                SharePreferencesUtil.putString(LoginActivity3.this,"rongYunKey",appKey);
                                SharePreferencesUtil.putString(LoginActivity3.this, "chatroomToken", token);
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                            String chatRoomId = jsonObject.getString("chatroomId");
                            SharePreferencesUtil.putString(LoginActivity3.this, "chatroomId", chatRoomId);
                            //是否是管理员 0 否1 是
                            String isSuperRoomManager = jsonObject.getString("isSuperRoomManager");
                            SharePreferencesUtil.putString(LoginActivity3.this,"isSuperRoomManager",isSuperRoomManager);
                            //融云id
                            String rcUsId = jsonObject.getString("rcUsId");
                            SharePreferencesUtil.putString(LoginActivity3.this,"rcUsId",rcUsId);
...
//                        ProgressDialogUtil.show(LoginActivity.this,Utils.getString(R.string.登录中),false);
                        ...
                        if(phone_login_constraintLayout.getVisibility() == View.VISIBLE){
                            ...
                            data1.put("refername",inviteCode);
                        }else {
                        ...
                        HttpApiUtils.CpRequest(LoginActivity3.this,null, RequestUtil.REQUEST_10r, data1, new HttpApiUtils.OnRequestLintener() {
                            @Override
                            public void onSuccess(String result, Headers headers) {
                                ...
                                if(phone_login_constraintLayout.getVisibility() == View.VISIBLE){
                                    //使用手机号一键登录时, 登录成功后保存手机号, 在认证页面需要将手机号写死
                                    SharePreferencesUtil.putString(MyApplication.getInstance(),CommonStr.USER_PHONE,phone_login_phone_etv.getText().toString());
                                }
                                RequestLoginUtils.requestSuccess(LoginActivity3.this,result,account, LoginActivity3.this.password,"");
                            }
