 ...
 */
@MessageTag(value = "s:close_live",flag = MessageTag.ISCOUNTED | MessageTag.ISPERSISTED)

public class CloseLiveMessage extends MessageContent {
    private String userInfoJson; //用户信息
    private String zkCode;//平台code

    public CloseLiveMessage(String zkCode) {
        this.zkCode = zkCode;
    }
     ...
     */
    public CloseLiveMessage(byte[] data) {
        String jsonStr = null;
    ...
    // Parcelable中 给消息赋值。
    public CloseLiveMessage(Parcel in) {
     ...
     */
    public static final Creator<CloseLiveMessage> CREATOR = new Creator<CloseLiveMessage>() {

        @Override
        public CloseLiveMessage createFromParcel(Parcel source) {
            return new CloseLiveMessage(source);
        }

        @Override
        public CloseLiveMessage[] newArray(int size) {
            return new CloseLiveMessage[size];
        }
    };
    ...
    public static Creator<CloseLiveMessage> getCREATOR() {
        return CREATOR;
    }
}
