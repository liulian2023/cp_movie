 ...
 */
@MessageTag(value = "s:follow",flag = MessageTag.ISCOUNTED | MessageTag.ISPERSISTED)

public class LiveFollowMessage extends MessageContent {
     ...
     */
    public LiveFollowMessage(byte[] data) {
        String jsonStr = null;
    ...
    // Parcelable中 给消息赋值。
    public LiveFollowMessage(Parcel in) {
     ...
     */
    public static final Creator<LiveFollowMessage> CREATOR = new Creator<LiveFollowMessage>() {

        @Override
        public LiveFollowMessage createFromParcel(Parcel source) {
            return new LiveFollowMessage(source);
        }

        @Override
        public LiveFollowMessage[] newArray(int size) {
            return new LiveFollowMessage[size];
        }
    };

    public LiveFollowMessage(String zkCode, String userInfoJson) {
        this.zkCode = zkCode;
