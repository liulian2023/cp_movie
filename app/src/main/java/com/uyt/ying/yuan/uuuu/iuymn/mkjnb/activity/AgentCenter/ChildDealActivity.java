...
public class ChildDealActivity extends BaseActivity implements TabLayout.BaseOnTabSelectedListener, View.OnClickListener {
    ...
    private   RecyclerView recyclerViewPup;//所有类型筛选popupwindow recycleView
    ...
    private EditText editText;//下级nickName输入框
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_child_deal);
        bindView();
        initView();
    ...
    private void bindView() {
        ...
        back=findViewById(R.id.child_deal_return);//返回键
    ...
    public void initView() {
        ...
        tab 导航栏相关配置
         */
        // private String[] titles = new String[]{Utils.getString(R.string.所有类型),Utils.getString(R.string.提现明细),Utils.getString(R.string.充值明细)};//tab Title数组
...
//        ArrayList<SoftReference<Fragment>> fragmentArrayList=new ArrayList<>();
        ...
        for (int i = 0; i < titles.size(); i++) {
            ...
            childDealFragment.setArguments(bundle);
//            fragmentArrayList.add(new SoftReference<>(childDealFragment));
            fragmentArrayList.add(childDealFragment);
        }
//        FragmentAdapter fragmentAdapter = new FragmentAdapter(getSupportFragmentManager(), titles, fragmentArrayList);
       ...
       筛选栏 recycleView 点击事件
        ...
        Utils.docking(dataAllType, RequestUtil.REQUEST_01dh, -1, new DockingUtil.ILoaderListener() {
            ...
            public void success(String content) {
                ...
                ChildBetAndDealAllPop childBetAndDealAllPop = new ChildBetAndDealAllPop(Utils.getString(R.string.全部类型), "", "", "");
                childBetAndDealAllPops.add(childBetAndDealAllPop);
                childBetAndDealAllPop.setStatus(1);//默认点击效果
                ...
                for (int i=0;i<zhaiyaoList.size();i++) {
                    ...
                    String type = jsonObject.getString("type");//type Tab切换和时搜索需要传入的数据
    ...
    public void onTabSelected(TabLayout.Tab tab) {
        if(tab.getText().equals(Utils.getString(R.string.所有类型))){
            ...
            initInStutas();
            initInTime();
            EventBus.getDefault().postSticky(new ChildDealEvenModel(user_id,pagenumAll,pageSizeAll,startAll,endAll,typeAll,initEdit(),startOut,endOut,stutasOut,startIn,endIn,stutasIn,false,false));
...
*/
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.child_deal_return:
                finish();
                break;
            //点击 弹出全部类型poppupwindow
            ...
            case R.id.choose_button_all:
                ProgressDialogUtil.show(ChildDealActivity.this);
                initAllTimeAndStutas();
                initOutStutas();
                initOutTime();
                initInStutas();
                initInTime();
//            getDataAll(user_id,pagenumAll,pageSizeAll,startAll,endAll,initEdit(),typeAll,false);
                EventBus.getDefault().postSticky(new ChildDealEvenModel(user_id,pagenumAll,pageSizeAll,startAll,endAll,typeAll,initEdit(),startOut,endOut,stutasOut,startIn,endIn,stutasIn,false,true));
            ...
            case R.id.choose_button_out:
                ...
                initInStutas();
                initInTime();
//            getDataOut(user_id,pagenumOut,pageSizeOut,startOut,endOut,initEdit(),stutasOut,false);
                EventBus.getDefault().postSticky(new ChildDealEvenModel(user_id,pagenumAll,pageSizeAll,startAll,endAll,typeAll,initEdit(),startOut,endOut,stutasOut,startIn,endIn,stutasIn,false,true));
...
//                ProgressDialogUtil.show(ChildDealActivity.this);
                ...
                initInStutas();
                initInTime();
//            getDataIn(user_id,pagenumIn,pageSizeIn,startIn,endIn,initEdit(),stutasIn,false);
                EventBus.getDefault().postSticky(new ChildDealEvenModel(user_id,pagenumAll,pageSizeAll,startAll,endAll,typeAll,initEdit(),startOut,endOut,stutasOut,startIn,endIn,stutasIn,false,true));
            ...
            case R.id.search_text:
                if(StringMyUtil.isEmptyString(initEdit())){
                    showToast(Utils.getString(R.string.请输入下级账号));
                }else{
                    initAllTimeAndStutas();
                    initOutStutas();
                    initOutTime();
                    ...
                    initInTime();
//                    ProgressDialogUtil.show(ChildDealActivity.this);
                    if(mtab.getTabAt(0).isSelected()){
                        EventBus.getDefault().postSticky(new ChildDealEvenModel(user_id,pagenumAll,pageSizeAll,startAll,endAll,typeAll,initEdit(),startOut,endOut,stutasOut,startIn,endIn,stutasIn,false,true));
                    }else if(mtab.getTabAt(1).isSelected()){
                        EventBus.getDefault().postSticky(new ChildDealEvenModel(user_id,pagenumAll,pageSizeAll,startAll,endAll,typeAll,initEdit(),startOut,endOut,stutasOut,startIn,endIn,stutasIn,false,true));
                    }else if(mtab.getTabAt(2).isSelected()){
                        EventBus.getDefault().postSticky(new ChildDealEvenModel(user_id,pagenumAll,pageSizeAll,startAll,endAll,typeAll,initEdit(),startOut,endOut,stutasOut,startIn,endIn,stutasIn,false,true));
                    }
                }
                break;
    ...
    private void initAllTimeAndStutas() {
        //每次请求网络数据时,初始化pagenum,避免在一个页面上拉加载完数据后,到下一页面pageNum还是上一个页面的值,
        pagenumAll=1;
        pagenumOut=1;
        pagenumIn=1;
        //evenBus在点击筛选按钮或者切换tab时,会先调用此方法,并发送事件, fragment接收到事件会去请求数据,所以在此方法中将refreshLayout的状态初始化,(避免上拉加载到最后,切换tab或者点击筛选按钮后,加载不出数据)
        ...
        if(count==1||childBetAndDealAllPops.size()==0){//默认选中第一个tab,网络异步请求,导致默认选中时,childBetAndDealAllPops还没有数据,空指针异常
            typeAll="";
        }else {
            typeAll=childBetAndDealAllPops.get(popPosition).getTypoeId();
        }
        count++;//每次选中第一个tab都让count递增,
    }
