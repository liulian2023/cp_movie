...
public class AnimationUtil {
    ...
    public static void slideButtons(Context context, final FloatingDraftButton button){
        ...
        int screenWidth = ScreenUtil.getWith(context);
        int buttonRight = screenWidth - button.getRight();
        int buttonTop = button.getTop();
        int buttonBottom = ScreenUtil.getHeight(context) - button.getBottom();
        int buttonWidth = button.getWidth();
        int radius = 7*buttonWidth/4;
        int gap = 5*buttonWidth/4;

        ArrayList<ImageView> buttons = button.getButtons();
        ...
        if(button.isDraftable()){//可拖拽 展开Buttons
            ...
            if(buttonLeft >= radius && buttonRight >= radius
                ...
                for(int i=0;i<size;i++){
                    ImageView faButton = buttons.get(i);
                    slideview(faButton,0, radius*(float)Math.cos(Math.toRadians(randomDegree+angle*i)),0,radius*(float)Math.sin(Math.toRadians(randomDegree+angle*i)),300,0,true,true);
                }
                ...
                if(buttonTop >= radius && buttonBottom >= radius){
                    if(buttonTop >buttonBottom){
                        ImageView faButton = buttons.get(0);
                        slideview(faButton,0,0,0,-radius,300,0,true,true);
                        for(int i=1;i<=leftNumber && i<size;i++){
                            faButton = buttons.get(i);
                            slideview(faButton,0, -gap*i,0, -radius,300,0,true,true);
                        }
                ...
                if((upNumber+belowNumber+1)>=size){
                    ...
                    if(buttonLeft >= radius){
                        ImageView faButton = buttons.get(0);
                        slideview(faButton,0, -radius,0,0,300,0,true,true);
                        for(int i=1;i<=upNumber && i<size;i++){
                            faButton = buttons.get(i);
                            slideview(faButton,0, -radius,0, -gap*i,300,0,true,true);
                        }
            ...
            for(ImageView faButton : buttons){
                int faButtonLeft = faButton.getLeft();
                int faButtonTop = faButton.getTop();
                slideview(faButton,0,buttonLeft-faButtonLeft,0,buttonTop-faButtonTop,300,0,true,false);
            }
        }
